...
import { PackagePlus, Save, Search, Plus, Minus, Package, History, Clock, FileText, Wallet, CreditCard, X, CheckCircle } from 'lucide-react';
...
const Inbound = () => {
  ...
  const [inboundSummary, setInboundSummary] = useState({ totalHpp: 0, items: [] });

  // Data Tab Inbound
  const products = useLiveQuery(() => db.products.orderBy("id").reverse().toArray(), []);
  ...
  // Data Tab Riwayat
  const inboundLogs = useLiveQuery(() => db.inbound_logs.orderBy("timestamp").reverse().toArray(), []);
  ...
  const handleUpdateQty = (productId, delta, newValue = null) => {
    setPendingUpdates(prev => {
      ...
      let newQty = newValue !== null ? parseInt(newValue) || 0 : currentQty + delta;
  ...
  const handlePreSave = async (e) => {
    ...
    if (totalItemsToUpdate === 0 || note.trim() === '') {
      showAlert('Isi kuantitas barang dan catatan nota terlebih dahulu!', 'error');
      return;
    }
    ...
    const productIds = Object.keys(pendingUpdates).map(Number);
  ...
  const handleConfirmInbound = async () => {
    if (paymentMethod === 'Hutang' && (!supplierName.trim() || !dueDate)) {
      showAlert('Nama supplier dan tanggal jatuh tempo wajib diisi untuk pencatatan hutang!', 'error');
      return;
    }

    setIsSubmitting(true);
    try {
      // ATOMIC TRANSACTION: Update stok, cetak log, dan potong uang di Pembukuan sekaligus!
      await db.transaction('rw', [db.products, db.inbound_logs, db.expenses, db.debts], async () => {
        ...
        for (const { product, qty } of inboundSummary.items) {
          ...
          // Update stok fisik di DB
          await db.products.update(product.id, { stok: newStok });
        }

        const timestamp = new Date().toISOString();

        // 1. Cetak dokumen Log History Inbound
        const logDocument = {
          timestamp,
        ...
        // 2. Integrasi ke Pembukuan (Akuntansi)
        // Hanya jika ada nilai uangnya (harga pokok > 0)
        if (inboundSummary.totalHpp > 0) {
          if (paymentMethod === 'Kas Tunai') {
            // Potong Kas Tunai sebagai Beban Modal Kulakan
            await db.expenses.add({
              amount: inboundSummary.totalHpp,
              description: `Kulakan Stok: ${note}`,
              timestamp: new Date().getTime(),
              synced: 0
            });
          } else if (paymentMethod === 'Hutang') {
            // Tambah Hutang di Buku Hutang
            await db.debts.add({
              supplier_name: supplierName,
              description: `Hutang Barang Masuk: ${note}`,
              amount: inboundSummary.totalHpp,
              paid_amount: 0,
      ...
      showAlert(`${totalItemsToUpdate} SKU berhasil di-inbound! Sistem pembukuan telah disinkronisasi.`, 'success');
      ...
      showAlert('Gagal menyimpan transaksi Inbound dan Akuntansi.', 'error');
    } finally {
    ...
    <div className="max-w-[1400px] mx-auto p-4 md:p-6 pb-24 animate-in fade-in slide-in-from-bottom-4 duration-300 h-full flex flex-col relative">
              ...
              <table className="w-full text-left border-collapse min-w-[700px]">
                <thead className="sticky top-0 bg-white shadow-sm z-10">
                  <tr className="border-b border-gray-200 text-sm text-gray-500 bg-gray-50">
                    ...
                    <th className="p-4 font-semibold text-center w-32">Stok Saat Ini</th>
                    <th className="p-4 font-semibold text-center w-48">Tambahan Masuk (+)</th>
                  </tr>
                </thead>
                <tbody>
                  {(!filteredProducts || filteredProducts.length === 0) ? (
                    ...
                    filteredProducts.map((p, index) => {
                      ...
                      return (
                        <tr key={p.id} className={`border-b border-gray-50 transition-colors ${isUpdated ? 'bg-primary-50/40' : 'hover:bg-gray-50'}`}>
                          ...
                          <td className="p-4">
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                <p className={`font-bold line-clamp-1 ${isUpdated ? 'text-primary-700' : 'text-gray-800'}`}>
                                  {p.nama}
                                </p>
                          ...
                          <td className="p-4">
                            <div className="flex items-center justify-center bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm focus-within:ring-2 focus-within:ring-primary-500/20 focus-within:border-primary-500 transition-all">
                              <button type="button" onClick={() => handleUpdateQty(p.id, -1)} className="w-10 h-10 flex items-center justify-center bg-gray-50 hover:bg-gray-200 text-gray-600 transition-colors border-r border-gray-200">
                                <Minus className="w-4 h-4" />
                              </button>
                              <input type="number" min="0" value={additionalQty === 0 ? '' : additionalQty} onChange={(e) => handleUpdateQty(p.id, 0, e.target.value)} placeholder="0" className="w-16 h-10 text-center font-bold text-primary-600 outline-none appearance-none bg-transparent" />
                              <button type="button" onClick={() => handleUpdateQty(p.id, 1)} className="w-10 h-10 flex items-center justify-center bg-primary-50 hover:bg-primary-100 text-primary-600 transition-colors border-l border-primary-100">
          ...
          <div className="w-full lg:w-80 flex flex-col gap-4 flex-shrink-0">
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 sticky top-6">
              ...
              <div className="bg-primary-50 border border-primary-100 rounded-xl p-4 flex flex-col items-center justify-center mb-5">
                <span className="text-sm font-semibold text-primary-600 mb-1">Total SKU Masuk</span>
                <span className="text-4xl font-bold text-primary-700">{totalItemsToUpdate}</span>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-md rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200 border border-gray-100">
            ...
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <h2 className="text-xl font-bold text-gray-800">Detail Pendanaan Inbound</h2>
              <button 
            ...
            <div className="p-6 flex flex-col gap-5">
              ...
              <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 flex flex-col items-center justify-center">
                <span className="text-sm font-medium text-blue-600 mb-1">Total Modal (Berdasarkan HPP)</span>
                <span className="text-3xl font-bold text-blue-700">{formatRupiah(inboundSummary.totalHpp)}</span>
              ...
              {inboundSummary.totalHpp > 0 && (
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-3">Sumber Dana / Pembayaran</label>
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                      ...
                      <span className="text-xs font-semibold text-center">Lunas (Potong Kas)</span>
                    </button>
                    <button
                    ...
                    >
                      ...
                      <span className="text-xs font-semibold text-center">Hutang ke Supplier</span>
                    </button>
                  </div>
                </div>
              )}
              ...
              {paymentMethod === 'Hutang' && inboundSummary.totalHpp > 0 && (
                <div className="animate-in slide-in-from-top-2 duration-300 space-y-3 bg-red-50/50 p-4 rounded-xl border border-red-100">
                  <div>
                    <label className="block text-xs font-bold text-gray-700 mb-1">Nama Supplier / Penagih *</label>
                    <input
                      type="text"
              ...
              {paymentMethod === 'Kas Tunai' && inboundSummary.totalHpp > 0 && (
                 <p className="text-xs text-gray-500 text-center bg-gray-50 p-2 rounded-lg">
                   Kas tunai di laporan Neraca akan berkurang sebesar {formatRupiah(inboundSummary.totalHpp)}.
                 </p>
              )}
            </div>
            ...
            <div className="p-4 border-t border-gray-100 bg-gray-50/50">
              ...
              >
                ...
                {isSubmitting ? 'Memproses...' : 'Konfirmasi & Tambah Stok'}
              </button>
            </div>
          </div>
        </div>
      ...
      {activeTab === 'history' && (
        <div className="flex-1 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col lg:flex-row animate-in fade-in min-h-0 mt-6">
          ...
          {/* List Riwayat (Kiri) */}
          <div className={`w-full ${selectedLog ? 'hidden lg:flex' : 'flex'} lg:w-1/3 flex-col border-r border-gray-100 h-full`}>
            ...
            <div className="flex-1 overflow-y-auto">
              {(!inboundLogs || inboundLogs.length === 0) ? (
                <div className="p-8 text-center text-gray-500">Belum ada rekam jejak inbound.</div>
              ) : (
                inboundLogs.map((log) => (
                  <div 
                    key={log.id} 
                  ...
                  >
                    <div className="flex justify-between items-start mb-1">
                      <span className="text-xs font-bold text-gray-400">{formatDate(log.timestamp)}</span>
                      <span className="text-[10px] bg-gray-100 px-2 py-0.5 rounded-full font-medium text-gray-600">ID: {log.id}</span>
                    </div>
                    <p className="font-bold text-gray-800 line-clamp-1 mb-1">{log.catatan}</p>
                    <div className="flex items-center gap-2 text-xs text-gray-500 mt-2">
                       <span className="font-medium text-primary-600 bg-primary-50 px-2 rounded">{log.total_sku} SKU</span>
                       <span className={`font-semibold px-2 py-0.5 rounded ${log.sumber_dana === 'Hutang' ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-600'}`}>
          ...
          <div className={`flex-1 flex-col h-full bg-gray-50/30 ${selectedLog ? 'flex' : 'hidden lg:flex'}`}>
             {!selectedLog ? (
               <div className="flex-1 flex flex-col items-center justify-center text-gray-400 p-8 text-center">
                 <FileText className="w-16 h-16 text-gray-200 mb-4" />
                 <p className="text-lg font-medium text-gray-500">Pilih riwayat di samping untuk melihat rincian barang masuk</p>
               </div>
             ) : (
               <div className="flex flex-col h-full animate-in fade-in duration-200">
                 ...
                 <div className="p-6 border-b border-gray-100 bg-white flex flex-col md:flex-row md:items-start justify-between shadow-sm z-10 gap-4">
                   <div>
                     <button onClick={() => setSelectedLog(null)} className="lg:hidden mb-4 text-sm text-primary-600 font-bold flex items-center gap-1 bg-primary-50 px-3 py-1.5 rounded-lg">
                        &larr; Kembali
                     </button>
                     <h2 className="text-2xl font-bold text-gray-800 mb-3">{selectedLog.catatan}</h2>
                     <div className="flex flex-wrap items-center gap-3 text-sm text-gray-500">
                        <span className="flex items-center gap-1.5 bg-gray-50 border border-gray-100 px-3 py-1.5 rounded-lg shadow-sm">
                          <Clock className="w-4 h-4 text-gray-400" /> {formatDate(selectedLog.timestamp)}
                        </span>
                        <span className="flex items-center gap-1.5 bg-gray-50 border border-gray-100 px-3 py-1.5 rounded-lg shadow-sm">
                          Oleh: <strong className="text-gray-700">{selectedLog.kasir_nama}</strong>
                        </span>
                     </div>
                   </div>
                   <div className={`p-4 rounded-xl border flex flex-col items-end ${selectedLog.sumber_dana === 'Hutang' ? 'bg-red-50 border-red-100' : 'bg-green-50 border-green-100'}`}>
                      <span className="text-xs font-semibold mb-1 uppercase tracking-wide opacity-70">Nilai Inbound</span>
                      <span className={`text-xl font-bold ${selectedLog.sumber_dana === 'Hutang' ? 'text-red-700' : 'text-green-700'}`}>
                        {formatRupiah(selectedLog.total_nilai || 0)}
                      </span>
                      <span className="text-xs font-medium mt-1">Pembayaran: {selectedLog.sumber_dana || 'Kas Tunai'}</span>
                   </div>
                 </div>
                 ...
                 <div className="flex-1 overflow-y-auto p-4 md:p-6">
                    <h3 className="font-bold text-gray-700 mb-4 text-sm uppercase tracking-wider flex items-center gap-2">
                       ...
                       <span className="bg-primary-100 text-primary-700 px-2 py-0.5 rounded-full text-xs">{selectedLog.items.length} Item</span>
                    </h3>
                    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                      <table className="w-full text-left border-collapse">
                        <thead className="bg-gray-50/80 border-b border-gray-100 text-xs text-gray-500 uppercase tracking-wider">
                          <tr>
                            <th className="p-4 font-semibold">SKU / Nama Produk</th>
                            <th className="p-4 font-semibold text-center w-24">Stok Awal</th>
                            <th className="p-4 font-semibold text-center w-24 text-primary-600">Masuk (+)</th>
                            <th className="p-4 font-semibold text-center w-24">Stok Akhir</th>
                            <th className="p-4 font-semibold text-right w-32">Total HPP</th>
                          </tr>
                        </thead>
                        <tbody>
                          {selectedLog.items.map((item, idx) => (
                            <tr key={idx} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/50 transition-colors">
                              <td className="p-4">
                                <p className="font-bold text-gray-800 text-sm mb-0.5">{item.nama}</p>
                                <span className="text-[11px] font-mono bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded">{item.barcode || 'NO-SKU'}</span>
                              </td>
                              <td className="p-4 text-center text-sm font-medium text-gray-500">{item.stok_sebelumnya}</td>
                              <td className="p-4 text-center text-sm font-bold text-primary-600 bg-primary-50/30">+{item.qty_masuk}</td>
                              <td className="p-4 text-center text-sm font-bold text-gray-800">{item.stok_sesudahnya}</td>
                              <td className="p-4 text-right text-sm font-bold text-gray-600">{formatRupiah(item.subtotal_hpp || 0)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
